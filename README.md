...
- [감사 후속 구현 기록](docs/reviews/2026-09-07-purpose-autonomy-efficiency-implementation.md): A01~A17 코드 반영과 검증·배포 증거를 분리한 종결표
...
자료 처리 → 근거 추출 → Essence 자동 검토·승인 → 일정·노출 타깃 계획 → 본문·이미지 생성 → 안전 검사 → 자동 발행이 콘텐츠 흐름이다. 작업 구현은 전체 자료를 durable batch로 회수하고, 콘텐츠 revision·검수 coverage·이미지 인증, 고정 반복 관측 슬롯, 공급자 HTTP 시도 원장을 추가한다. 정상 생성·측정·발행·자동 복구는 Slack으로 매번 알리지 않는다. 운영 공개 글 115건의 전환은 API 배포 차단 조건이다. FAQ 구두점 3건의 제한된 수정, 미해결 AI 검수 22건의 독립 재검수, 115건 전체 이미지의 실제 바이트 인증·불변 사본 저장을 순서대로 종결한 뒤 rollout한다.
...
배포 진입점은 `bash scripts/deploy.sh all`이며, API 배포는 Worker/Beat 호환성·마이그레이션·RedBeat 재조정·준비 검사를 함께 고려한다. 상세 순서와 롤백 한계는 [배포 안내](docs/ops/deployment-runbook.md), 마지막으로 완료가 확인된 운영 상태는 [2026-09-07 릴리스](docs/releases/2026-09-07-345a642.md)에 기록했다. 현재 작업 구현은 이 릴리스 뒤 변경이며 배포 완료로 해석하지 않는다.
