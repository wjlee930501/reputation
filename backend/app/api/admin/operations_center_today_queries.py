...
from sqlalchemy import String, false, func, literal, select
...
# 이 큐에 남는 유일한 상태. 공개 후 확인 표본은 발행을 막지 않는 관측용 표본이고 두 번째
...
async def load_today_queue(
    ...
    *,
    ...
    """Load due publishing work for one page.

    Selection, counting and ordering stay set-based over one state. The post-publish
    human sample never enters this queue — it is observability sampling, so it lives in
    the content tab filters and in monthly report evidence, not in an operator queue.
    Blocked published images stay operator work through their own incidents, which the
    incident queue owns.
    ...
    if filters.status:
        predicates.append(literal(_PUBLISH_DUE) == filters.status)
    if filters.severity:
        predicates.append(literal(_MEDIUM) == filters.severity)
    match filters.sla:
        case None:
            pass
        case SlaFilter.OVERDUE:
            ...
            predicates.append(ContentItem.scheduled_date < today)
        case SlaFilter.DUE:
            predicates.append(ContentItem.scheduled_date >= today)
        case SlaFilter.NONE:
            # 발행 예정 슬롯의 기한은 언제나 예정일이 끝나는 시각이다 — 기한 없는 행이 없다.
    ...
    page_stmt = (
        query.order_by(ContentItem.scheduled_date, ContentItem.id)
    ...
    impact, next_action = _today_operator_copy()
    items: list[OperationsQueueRow] = []
    for content, hospital, handoff, actor, run, incident, _total in rows:
        # 이 행의 기한은 콘텐츠 작업의 기한이다. 예전에는 계약 인수 기한을 보여 주면서
        # 상태는 다른 기준으로 정해, 서로 다른 두 기한이 한 줄에 섞였다(G-2).
        ...
        items.append(
            OperationsQueueRow(
                ...
                requires_operator_action=publish_due_requires_operator_action(
