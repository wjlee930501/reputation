...
from app.services.sov_trend import (
    _ambiguous_clause,
    _confirmed_clause,
    _failed_clause,
    _mentioned_clause,
    weekly_mention_trend,
)
...
MEASUREMENT_METHOD_DISPLAY_LABELS = {
    ...
    "OPENROUTER_CHAT_COMPLETIONS": "OpenAI 모델 응답 측정",
    "OPENROUTER_RESPONSES_WEB_SEARCH": "ChatGPT Search 유사 측정",
    "OPENROUTER_GEMINI_WEB_SEARCH": "Gemini 검색 측정",
}
...
async def get_sov_trend(hospital_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    """
    await _get_hospital_or_404(db, hospital_id)
    return await weekly_mention_trend(db, hospital_id)
