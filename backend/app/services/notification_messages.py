...
def _incident_notification(
    ...
    if recovered:
        ...
        context = " · ".join(labels)
    message = _message(
        f"{title} | {problem} | {action}",
        (
            _block("header", "header", {"type": "plain_text", "text": title}),
...
def build_summary_notification(
    ...
    blocks = (
        _block("header", "summary_header", {"type": "plain_text", "text": f"[조치 필요] 운영 이슈 {len(ordered)}건"}),
        _block(
            "section",
            "summary_window",
            {
    ...
    return NotificationIntent(
        ...
        message=_message(
            f"[조치 필요] 운영 이슈 {len(ordered)}건 | " + " / ".join(
