...
def collect_fleet_facts(db, *, now):
    ...
    return FleetFacts(hospitals, recovering, operator_work, failed, measured_count, reports)
...
def build_fleet_heartbeat(report: WatchdogReport, facts: FleetFacts, *, now, admin_base_url):
    ...
    degraded = (
        ...
        or facts.failed_runs > 0
        or facts.operator_work > 0
    )
    state = "관측 부족" if unknown else "미완료 항목 있음" if degraded else "관측 지표 양호"
    ...
    text = "\n".join(
        [
            ...
            f"자동 복구 {facts.recovering}건 · 사람의 개입 {facts.operator_work}건 · 최근 24시간 실패/부분 완료 작업 {facts.failed_runs}건",
