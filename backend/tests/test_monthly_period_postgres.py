"""Real PostgreSQL proofs for historical eligibility and version serialization."""
...
def test_real_postgres_historical_eligibility_and_concurrent_versions() -> None:
    schema = f"task21_{uuid.uuid4().hex}"
    ...
    eligible = models.Hospital(
        name="8월 서비스 의원", slug=f"month-eligible-{uuid.uuid4().hex}"
    )
    ended = models.Hospital(
        name="7월 종료 의원", slug=f"month-ended-{uuid.uuid4().hex}"
    )
    ...
    try:
        ...
        with Session(engine) as db:
            v1 = db.scalars(
                select(models.MonthlyReport).where(
                    models.MonthlyReport.hospital_id == eligible_id
                )
            ).one()
