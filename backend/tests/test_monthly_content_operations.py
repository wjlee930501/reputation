    ...
    *,
    ...
    return SimpleNamespace(
        ...
        essence_check_summary={
            "automatic_remediation_attempts": remediation_attempts,
        },
        body_updated_at=body_updated_at,
    )


def test_monthly_content_operations_blocks_pending_required_review_sample():
    cutoff = datetime(2026, 8, 1, 0, 15, tzinfo=timezone.utc)
    reviewed = _item(reviewed_at=datetime(2026, 7, 10, 1, 0, tzinfo=timezone.utc))
    pending = _item(sequence_no=2, remediation_attempts=1)
    ...
    assert not any("약정 콘텐츠" in blocker for blocker in snapshot.delivery_blockers)
    assert any("필수 사후검수 샘플 1건" in blocker for blocker in snapshot.delivery_blockers)
...
def test_closed_month_shortfall_is_reported_without_making_delivery_impossible():
    ...
    assert "복구 계획" in snapshot.payload["operator_copy"]["next_action"]
