...
class _RecoverySession:
    def __init__(self, *, hospitals=(), runs=()):
...
def test_reconciler_requeues_stranded_site_build_and_revalidation(monkeypatch) -> None:
    ...
    dispatched: list[tuple[str, list[str | int]]] = []
    ...
    assert result == {"site_builds": 1, "site_revalidations": 1}
    assert dispatched == [
        ("app.workers.tasks.build_aeo_site", [str(hospital.id)]),
        ("app.workers.tasks.retry_site_revalidation", [str(run.id), 0]),
    ]
