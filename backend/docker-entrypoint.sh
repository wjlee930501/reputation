...
case "$SERVICE" in
  ...
  inspect-schema)
    ...
    ;;
  force-hard-rewrite)
    # CONTENT_AI_HARD_FINDING으로 막힌 슬롯의 강제 재작성 적재 — Cloud Run Job.
    # FORCE_HARD_REWRITE_CONTENT_IDS에 글 id를 넣고, 실제 적재는 --apply를 붙일 때만 한다.
    # 발행 권한은 달라지지 않는다 — 재작성 결과는 기존 발행 게이트가 그대로 판정한다.
    exec python -m app.utils.force_hard_finding_rewrite ${FORCE_HARD_REWRITE_APPLY:+--apply}
    ;;
  *)
    echo "Unknown SERVICE: $SERVICE"
    echo "Valid values: api, worker, beat, flower, migrate, seed-admin, backfill-images, seed-colon-cluster, unpublish-flagged, fix-director-credential, inspect-schema, force-hard-rewrite"
    exit 1
    ;;
esac
