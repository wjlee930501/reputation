...
- 이미지는 업로드 전 정책 검토를 거치며 새 발행은 이미지 내용 hash·주제 hash·정책 버전에 연결된 인증을 요구한다. 생성 인물을 실제 원장 신원으로 사용하지 않는다. 운영 공개 글 115건의 기존 이미지는 배포 전 일회성 Job/CLI가 실제 바이트 재검수·content-addressed 불변 사본·CAS 저장으로 이 계약에 편입한다. 공개 GCS 이미지 프록시 URL은 인증된 내용 hash를 버전 질의값으로 포함해야 하며, 이미지 교체 때 그 값도 바뀌어 Site 이미지 캐시가 이전 바이트를 계속 제공하지 않게 한다. 기존 검수 시각만 인정하는 호환은 복구 중인 구 API 전환 구간에만 허용하고 새 API gate에는 남기지 않는다. 비공개 기존 행도 일반 생성·발행의 엄격한 인증 gate를 통과해야 한다. 영구 레거시 우회나 합성 인증값을 만들지 않는다.
...
- 기존 공개 글의 전환은 고정된 운영 manifest로만 수행한다. 현재 preflight는 115건 중 AI 검수 미해결 22건(REVISE 19, UNAVAILABLE 3), AI 검수 레거시 기준 허용 93건(ABSENT 92, PASS 1), FAQ 구두점 수정 3건이다. FAQ 수정은 질문 끝 물음표만 CAS로 바꾸고 수정 직후 독립 검수 대기(DEFERRED)로 남겨 다음 22건 재검수 단계에서 처리한다. ABSENT/PASS 93건은 AI 검수 메타데이터가 레거시라는 이유만으로 유료 재검수하지 않는다. 콘텐츠·이미지 전환 뒤 정확한 115개 ID가 새 엄격 공개 gate를 통과하는지 read-only 검증한다.
...
- 월간/V0 고정 관측 슬롯의 질문·플랫폼·반복·protocol과 답변·판정 체크포인트를 보존한다. COMPLETE/LIMITED/UNAVAILABLE와 legacy lineage를 구분하며 비교 불가한 기간을 상승/하락으로 표현하지 않는다.
