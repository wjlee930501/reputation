...
[STEP 6] 콘텐츠 운영 기준 자동 준비 및 스케줄 설정 (시스템 기본, 사람은 예외만)
    ...
    • 자료가 추가·수정되면 기존 승인은 stale 처리되며 새 승인 전에는 생성·발행하지 않는다
    ↓
[STEP 7] 콘텐츠 자동 생성 사이클 (시스템 — 이후 지속)
    발행일 전날 밤 23:00
        → 병원별 최신 승인 콘텐츠 운영 기준(STEP6) 확인 — 없으면 생성을 건너뛰고 Slack 알림
    ...
    ↓
[STEP 8] 월간 AI 답변 언급 리포트 (시스템 — 다음 달 1일 00:15 마감, 7일까지 자동 복구)
