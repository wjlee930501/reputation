...
def test_admin_surfaces_use_the_unified_terms() -> None:
    """admin 화면은 검토 §4에서 하나로 묶은 용어만 쓴다.

    옛 8개 화면이 사라진 뒤로는 경로 허용 목록 없이 `admin/app`·`admin/lib`·
    `admin/types` 전체를 본다. 패턴 자체가 사라지지 않았는지도 확인한다 — 패턴이 비면
    가드가 조용히 아무것도 안 지키면서 계속 초록이다.
    """
    assert NEW_SURFACE_BANNED_PATTERNS, (
        "NEW_SURFACE_BANNED_PATTERNS가 비었다. 용어 가드가 아무것도 검사하지 않는다."
    )

    violations = unified_term_violations()
    assert not violations, (
        "admin에 통일 전 용어가 남았다. admin/lib/admin-copy.ts의 키로 바꿀 것:\n"
        + "\n".join(violations)
    )
