...
## Source of truth
- Status: Active
- Last refreshed: 2026-09-15 (병원 홈 레이아웃 시스템 재정립)
...
- Collection previews are bounded: home gallery 6 images by default and never more than 8, ordered 외관 → 내부 → 진료·시술실 and excluding the photo already used as the hero; larger libraries disclose a count and link to `/visit#gallery` rather than rendering every asset in the reading flow. The grid shape is decided by the count (`data-count`) so no count strands a single tile: 1–2/4 → 2 equal columns, 3/6 → 3 columns with a 2×2 lead, 5 → 4 columns with a 2×2 lead, 7 → 4 columns with a lead and two 2-wide closers, 8 → 4 equal columns; below 1024px always 2 columns, lead spanning only for odd counts.

## Visual Language
- Direction: restrained local-clinic editorial on pure white. White paper (`#FFFFFF`, landing과 같은 기준), neutral hairlines (`--clinic-line-soft`), strong Korean type and verified photography carry identity; the only colour is the hospital's approved brand ramp. The memorable moment is the first viewport's truthful pairing of care scope with immediate access information—not decoration.
...
- Typography (Pretendard only; weights 400/500/600/700): editorial hierarchy is built from scale + weight, not new fonts. Hero display `.hub-hero-title` 44 / 36 / 30px (`--clinic-type-display`) with the emphasised part in the brand action colour. Section titles `.hub-section-title` 28 / 22px (`--clinic-type-h2`), h3 18px, lede 17px, body 16px, small 14px, caption 13px — all from `--clinic-type-*` tokens. Article title 40px / 27px. Article body 17px / line-height 1.8, measure ~680px. Body/notes ≥16px, mobile included.
- Visual anchors (v7) — every section owns one dominant anchor: hero = verified facility photograph bleeding to the right viewport edge above the 1296px baseline, or a typographic access panel; facts = today's hours + phone + address rail; treatments = ordered directory; doctor = one card per physician, verified portrait or typographic monogram with credentials; featured = one accountable lead plus density-aware support; gallery = curated 6–8 approved facility photos with a two-column lead tile at ≥1024px; visit = three direct actions plus transport/preparation details and the full category-grouped gallery. No generated person is a trust anchor.
- Rhythm — editorial density over card sprawl: each section changes layout so nothing is a repeated card grid. Section backgrounds alternate white ↔ `--clinic-surface-alt` **by document order** (`.hub-main > .hub-section:nth-of-type(even)`), never by component, so a hidden section cannot put two grey bands next to each other. Structure is expressed with hairline dividers + whitespace; boxes (`.hub-card`) are used only where they earn hierarchy (treatment cards, physician cards, calendar, contact).
- Layout system (2026-09-15, `site/app/[slug]/_styles/`): one container (`.hub-container`: `--clinic-max` 1200px + `--clinic-gutter` 24/20px) shared by header, hero, every section and the footer, so the page has a single left alignment line. Spacing comes only from the 4px scale tokens (`--clinic-space-1…16`), section rhythm from `--clinic-section-y` (96/72/56), head-to-body from `--clinic-section-head-gap`, card padding/gaps from `--clinic-card-pad`/`--clinic-block-gap`. Each `hub-*` selector lives in exactly one file (`tokens → base → header → hero → treatments → doctor → principles → content → flow → gallery → facts → footer`); the layout imports them in that order and `clinic-visual-system.test.ts` guards order, single ownership, token-only spacing, flatness and the accent 5% rule. Sub-pages (`/doctor`, `/treatments`, `/contents`, `/visit`, article) still use the legacy `.clinic-*` rules that remain in `globals.css` (dead rules purged 2026-09-15: 10,126 → 5,882 lines) — migrating them onto the `hub-*` layer is the next step.
...
- `ClinicHeader` (v8): one row — brand | section nav | phone CTA — with the brand visible at every width; below 720px the nav becomes a one-line horizontal scroll under the brand and the fixed bottom action bar (전화/진료시간/진료안내/길찾기) appears. The specialty·region subline was dropped from the header (the hero kicker says it).
- `ClinicHero` (v7): 5/7 split inside the shared container — kicker, H1 (≤2 specialties, `text-wrap: balance`), lede, two CTAs; verified media in a 4:3 rounded frame (16:10 stacked below 1024px). Below it a four-cell fact row (오늘 진료 · 전화 · 위치 · 토요일/다음 진료) on one hairline-bounded grid (`1fr 1fr 1.5fr 1fr`, 2×2 at tablet, stacked on mobile). The separate section-index band was removed — the header nav already covers it. No-media state is a quiet tinted panel with the hospital name.
- `HospitalFacts` (v5, 진료시간·오시는 길): the one place on the home where address, phone and hours appear in full (the hero row is a summary, the footer is the legal line; the old home repeated them four times). 7/5 split: weekly calendar card (`.hub-week`, KST-today highlighted, lunch on a second line, 휴진 notice; a row list on mobile) beside a contact card (phone / address + 길찾기 / 진료영역·지역), then official channel chips + HIRA line and the three visit checks (주차·대중교통·초진 준비물). Home no longer renders `ContactCard`; it remains the `/visit` component.
...
- `TreatmentGrid` (v5): up to 4 equal-height lead cards (`.hub-tx-card`: index in accent, name with a reserved right gutter, 2-line clamped description, chevron in a fixed top-right slot so it never collides with the name), column count from item count (`--clinic-tx-columns`), then a `hub-more` link with the total.
- `CareFlow` (v5): four steps under a 2px top rule with a small numbered circle (accent), aux marker, title, body; 4 → 2 → 1 columns.
- `DoctorIntro` (v8): one card per physician from the ordered `physicians[]` payload (older payloads synthesize a single physician from `director_*`). Layout follows the count — 1 = 320px portrait / body split, 2–3 = two columns, 4+ = three columns; two columns at ≤1023px, one column with a 1:1 portrait at ≤719px. Each card: 4:5 verified real portrait over a permanent monogram, `대표원장` eyebrow only for a representative, the 직함 as the single role label, ≤6 specialty chips, credential groups as dot-separated inline lists, and a career folded behind a native `<details>` "약력 더보기" above 280 characters. The `원장` honorific carries its own leading space so extracted text never reads `전상훈원장`. The 의료 정보 글 / 담당 진료 / 진료 지역 meta strip was removed (it printed `0편` for new hospitals and repeated the hero kicker).
- `FeaturedContent` / `/contents` featured / article header: `ContentCover` banner (real image or type motif, never an empty box).
- `ContactCard` (v4, `/visit` only): action-first 방문 안내 — three action buttons (전화하기 / 길찾기 / 진료시간 보기), a location frame, three visit checklists (주차 / 대중교통 / 초진 준비물), then official channel cards.
...
- Test/screenshot expectations: run Site tests/lint/build/typecheck plus the six hospital data-stress fixtures. The clinic style layer is read by `lib/clinic-stylesheet.ts` (globals.css + `_styles/*` in import order) and guarded by `clinic-container-width`, `clinic-tap-target`, `clinic-visual-system`, `clinic-surface-tokens` and `clinic-section-header` tests — new `hub-*` controls must be added to the tap-target list, new spacing must come from tokens, and a selector may live in one style file only. Aside Browser captures every public route for all six live hospitals at 390/768/1440 after the final edit; independent visual QA checks theme differentiation, CJK wraps, H1=1, page/table overflow, media provenance state, CTA availability and sparse/rich composition.
