...
- `DoctorIntro` (v7): one card per physician from the ordered `physicians[]` payload (older payloads synthesize a single physician from `director_*`). Layout follows the count — 1 = portrait/body split, 2–3 = two columns, 4+ = three columns on desktop, two at ≤1024px, portrait-left rows at ≤720px. Each card: 4:5 verified real portrait over a permanent monogram, `대표원장` eyebrow only for a representative, the 직함 as the single role label (a role equal to the eyebrow renders once), ≤6 specialty chips, credential groups, and a career folded behind a native `<details>` "약력 더보기" above 280 characters. The `원장` honorific carries its own leading space so extracted text never reads `전상훈원장`. The 의료 정보 글 / 담당 진료 / 진료 지역 meta strip sits once below the grid.
