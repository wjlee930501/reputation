...
문서 버전: **1.3** · 갱신일: **2026-09-07 (Asia/Seoul)**
소스 기준선: **`39dc1f8a98abe9193a8e2202395d2272c370fe8e`**
구현 상태: **기준선 위 작업 트리 로컬 검증·독립 리뷰 완료. 커밋·운영 전환·배포 증거 대기**

이 문서는 [원본 감사](2026-09-07-purpose-autonomy-efficiency-audit.md)의 A01~A17을 없었던 문제처럼 덮어쓰지 않고, 후속 구현과 검증 증거를 연결한다. 동결된 Backend 작업 트리는 전체 로컬 검증과 독립 코드·구조 리뷰를 통과했다. 이는 유료 운영 backfill, 운영 DB migration, RedBeat 재조정, queue canary, Cloud Run 배포와 공개 표면 검증을 뜻하지 않는다. 최종 커밋 SHA와 운영 revision·image digest는 릴리스 때 채운다.

## 구현 종결표

| 항목 | 작업 트리에서 확인한 구현 | 현재 판정 | 최종 종결에 필요한 증거 |
|---|---|---|---|
| A01 콘텐츠 독립 검수 | HARD/SOFT/UNCERTAIN finding, 전체 공개 후보 hash·필드별 coverage, 수정 후보 재검수, blocking finding 발행 차단. 운영 preflight는 미해결 22건(REVISE 19, UNAVAILABLE 3)과 AI 검수 레거시 기준 허용 93건(ABSENT 92, PASS 1)을 분리했고 exact allowlist 재검수 도구를 추가했다 | **로컬 검증 통과·운영 재검수 대기** | FAQ 구두점 CAS 뒤 exact 22건 독립 재검수 종결. AI 검수 레거시 93건은 과거 상태만으로 재검수하지 않고 전체 전환 뒤 exact ID 새 엄격 공개 gate 검증 |
...
| A03 늦은 생성 결과·발행 경합 | `content_revision`·`generation_claim_token` 조건부 저장, 이미지 내용/주제 hash·정책 버전 인증, 발행 잠금 안 현재 예정일 재확인. 레거시 FAQ 3건은 exact old question·revision·후보 hash plan을 한 트랜잭션 CAS로 물음표만 수정하고 검수 대기로 남김 | **로컬 검증 통과·운영 FAQ 수리 대기** | FAQ exact 3건 운영 dry-run·CAS apply·재실행 무변경 증거 |
...
| A12 이미지 단계 분리 | 신규 생성의 생성 바이트·정책 검수·업로드 재시도 분리와 내용/주제/정책 인증, 운영 115건의 실제 바이트 재검수·불변 사본·CAS writeback·안전하지 않은 이미지 교체 Job/CLI | **로컬 검증 통과·운영 인증/배포 차단** | 배포 전 exact 115건 manifest를 제한된 batch로 실행해 실제 바이트 인증·불변 URL 또는 안전한 교체를 모두 종결 |
...
- [기존 공개 본문 재검수](../../backend/app/services/content_public_review_backfill.py): exact allowlist만 제한된 batch로 재검수하고 후보·brief·현재 Essence·source snapshot·revision이 바뀌면 저장하지 않는다. 로컬 검증을 통과했고 운영 실행은 대기다.
- [기존 이미지 인증](../../backend/app/services/content_image_certification.py): exact manifest의 실제 바이트 검수·불변 사본·CAS 저장과 안전하지 않은 이미지 교체를 수행한다. 로컬 검증을 통과했고 운영 실행은 대기다.
- 세 배포 전 Job/CLI는 새 schema를 요구하지 않는다. 이번 변경의 expected head는 `0068_lead_cost_deferral`이다.
...
| 증거 | 상태 | 결과 |
|---|---|---|
| Backend 전체 단위·통합 테스트 | **통과** | 3,079 passed, 0 failed, 0 skipped, 18 warnings, coverage 83.06%, 95.36초. 실제 PostgreSQL·Redis·native PDF, fresh schema와 populated `0064` → `0068` upgrade 포함 |
| Admin test·lint·typecheck·production build | 로컬 검증 통과·CI build 대기 | 전체 536/536 tests·typecheck·lint 통과 뒤 마지막 raw fallback 수정의 관련 19 tests·typecheck도 통과. 앞선 production build는 통과했으며 최종 소스 CI lint·build 대기 |
| Site test·lint·typecheck·production build | 로컬 검증 통과·Linux build 대기 | 310 tests, typecheck, lint 통과. 로컬 production build는 EPERM으로 미완료, Linux CI 대기 |
| Ruff·diff·copy guard·DB connection budget guard | **통과** | Ruff, diff check, copy guard 통과. DB 연결 예산 75/80 |
| 독립 코드·구조 리뷰 | **통과** | code APPROVE, architecture CLEAR, P0/P1 0건 |
| Alembic upgrade 및 최종 current head | 로컬 통과·운영 대기 | expected head `0068_lead_cost_deferral`. fresh DB와 데이터가 채워진 `0064` DB의 upgrade 통과, 운영 DB current 확인·적용 대기 |
...
| 7개 queue canary·서명 dispatch·시간 제한 | 로컬 계약 통과·운영 대기 | 운영 Worker 배포 뒤 현재 release의 canary·queue wait 확인 대기 |
| 실제 배포 Git SHA·image digest·Cloud Run revision | 대기 | — |
| sitemap·llms.txt·공개 콘텐츠·이미지·IndexNow 복구 관측 | 대기 | — |
| provider usage 원장·spool·비용 영수증 운영 관측 | 대기 | — |
| Slack 정상 무알림·최종 차단 중복 억제 | 대기 | — |
| 운영 preflight baseline | read-only 확인 | 공개 115건. AI 검수 미해결 22건(REVISE 19, UNAVAILABLE 3), 레거시 기준 허용 93건(ABSENT 92, PASS 1), FAQ 구두점 대상 3건, 이미지 결합 인증 없음 109건·검수 시각만 있음 6건. mutation 실행은 대기이며 ID는 문서에 나열하지 않음 |
| FAQ 구두점 exact repair | **배포 차단** | exact 3건 dry-run plan·CAS apply·재실행 안전성·독립 검수 대기 확인 전 다음 단계 금지 |
| 공개 본문 독립 재검수 | **배포 차단** | FAQ 수리 뒤 exact 22건 allowlist의 cleared/blocked/unavailable/cost-blocked/stale 종결과 실제 후보 결합 증거 대기 |
| 운영 기존 이미지 인증 backfill | **배포 차단** | 공개 글 exact 115건 전체를 실제 바이트로 재검수한다. 109건 인증 공백과 6건 검수 시각 전용 상태를 불변 사본·CAS 저장 또는 안전한 교체로 종결하기 전 API rollout 금지 |
...
운영 데이터 점검에서 공개 글 115건 중 기존 이미지 109건에 새 결합 인증이 없고 6건은 기존 검수 시각만 있는 것을 확인했다. 영구적인 레거시 우회나 합성 인증값을 넣지 않는다. 배포 전 일회성 Job/CLI로 115건 모두의 실제 저장 바이트를 정책 재검수하고 content-addressed 불변 사본으로 옮기거나 안전한 이미지로 교체한 뒤 현재 revision·claim·URL을 비교하여 저장한다. 기존 검수 시각 호환은 복구 중인 구 API 전환 구간에만 허용하고 새 API에는 남기지 않는다. 비공개 기존 행도 일반 생성·발행의 엄격한 gate를 따른다. 이 작업의 종결 증거가 남을 때까지 API 배포는 차단한다.

staged 실행 순서는 migration `0068` → Worker → FAQ exact 3건 dry-run·CAS apply → 공개 본문 exact 22건 독립 재검수 → 이미지 exact 115건 실제 바이트 인증·불변 복사·필요 시 교체 → 레거시 이미지 시각 우회가 없는 새 엄격 공개 gate의 exact baseline ID read-only 검사 → RedBeat `2026-09-07.2` 재조정·Beat·readiness → API·Site·Admin이다. 구 API의 시각 기반 허용은 이미지 복구 중 전환 구간에만 사용하고 새 API에는 남기지 않는다. 아직 어느 운영 변경도 실행 완료로 기록하지 않는다. 상세 ID와 단계별 JSON·로그는 추적하지 않는 `/private/tmp/reputation-autonomy-release` 아래에 두고 이 문서에는 집계와 최종 판정만 남긴다.
