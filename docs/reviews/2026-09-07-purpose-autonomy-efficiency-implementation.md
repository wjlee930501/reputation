...
문서 버전: **1.5** · 갱신일: **2026-09-07 (Asia/Seoul)**
소스 기준선: **`39dc1f8a98abe9193a8e2202395d2272c370fe8e`**
구현 상태: **기준선 위 최종 작업 트리 로컬 검증·독립 리뷰 완료. 최종 결합 커밋 CI·운영 전환·배포 증거 대기**

이 문서는 [원본 감사](2026-09-07-purpose-autonomy-efficiency-audit.md)의 A01~A17을 없었던 문제처럼 덮어쓰지 않고, 후속 구현과 검증 증거를 연결한다. `0069` 최초 공개 이력과 이미지 URL cache key를 포함한 동결 Backend 작업 트리는 전체 로컬 검증과 독립 코드·구조 리뷰를 통과했고, 초기 구현 커밋 `8bc8353`은 Draft PR #82에 올라갔다. 이는 최종 결합 커밋 CI나 유료 운영 backfill, 운영 DB migration, RedBeat 재조정, queue canary, Cloud Run 배포와 공개 표면 검증을 뜻하지 않는다. 최종 소스 SHA와 운영 revision·image digest는 릴리스 때 채운다.
...
| A12 이미지 단계 분리 | 신규 생성의 생성 바이트·정책 검수·업로드 재시도 분리와 내용/주제/정책 인증, 운영 115건의 실제 바이트 재검수·불변 사본·CAS writeback·안전하지 않은 이미지 교체 Job/CLI. 공개 GCS 이미지 프록시는 인증 내용 hash를 URL 버전으로 사용해 교체된 바이트의 Site 이미지 캐시 key도 바꿈 | **로컬 검증 통과·운영 인증/배포 차단** | 배포 전 exact 115건 manifest를 제한된 batch로 실행해 실제 바이트 인증·불변 URL 또는 안전한 교체를 모두 종결하고, 교체 전후 프록시 `?v=`와 인증 hash 일치 확인 |
...
| 초기 구현 커밋 Linux CI | **통과** | Draft PR #82의 `8bc8353`에서 9/9 통과. Backend job 4분 33초, Admin·Site tests·typecheck·lint·production build, Backend/Admin/Site Docker build, security·Terraform·guard 포함. 후속 `0069`·이미지 URL cache key 수정 결합 CI는 별도 대기 |
| Admin test·lint·typecheck·production build | **초기 커밋 Linux CI 통과** | `8bc8353`: tests·typecheck·lint·production build 통과. 로컬 전체 536/536 뒤 마지막 raw fallback 수정의 관련 19 tests·typecheck도 통과. 후속 `0069` 결합 CI 대기 |
| Site test·lint·typecheck·production build | **초기 커밋 Linux CI 통과** | `8bc8353`: tests·typecheck·lint·production build 통과. 로컬 310 tests·typecheck·lint 통과. 후속 `0069` 결합 CI 대기 |
...
| Alembic upgrade 및 최종 current head | **로컬 통과·운영 적용 대기** | expected head `0069_content_first_publication`. fresh DB와 데이터가 채워진 `0064` → `0069` upgrade 통과, 운영 DB current 확인·적용 대기 |
...
| 운영 기존 이미지 인증 backfill | **배포 차단** | 공개 글 exact 115건 전체를 실제 바이트로 재검수한다. 109건 인증 공백과 6건 검수 시각 전용 상태를 불변 사본·CAS 저장 또는 안전한 교체로 종결하고, 프록시 `?v=`가 인증 내용 hash와 일치하며 교체 때 바뀌는지 확인하기 전 API rollout 금지 |
