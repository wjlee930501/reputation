...
## 0. 현재 상태 (2026-07-30 확인)

| | 상태 |
|---|---|
| 코드 | `main`에 머지·푸시 완료 |
| **프로덕션 배포** | **안 됨** — `https://reputation.motionlabs.kr/ai-diagnosis`가 404 |
| 프로덕션 DB 마이그레이션 | `0035`·`0036` 미적용 |
| Secret Manager | 신규 3종 미등록 |
| gcloud 인증 | **만료** — 재로그인 필요 |

이 저장소는 **자동 배포(CD)가 없다.** `main`에 푸시해도 아무것도 나가지 않고,
`scripts/deploy.sh`를 사람이 직접 실행해야 배포된다. 지금 살아 있는 사이트는 이전 리비전이다.
