...
| | 결과 |
|---|---|
| 시크릿 | `LEAD_LOCK_HASH_PEPPER`·`LEAD_REPORT_TOKEN_SECRET` 생성 + 권한 부여. `RESEND_API_KEY`는 **이미 존재**했다 |
...
| **실제 신청 1건 (메일 도착)** | ⏳ **미실행** — 아래 참조 |

`실제 신청 1건`은 영구 부작용이 있어 남겨뒀다: 쓰는 전화번호·이메일이 **영구 잠금**되고
(해제는 Admin API), 외부 API 비용이 실제로 나가고, 메일이 실제로 발송된다.
**메일 경로는 이번에 처음 프로덕션에 나간 코드라 아직 한 번도 실행되지 않았다** —
이 릴리스의 가장 큰 미검증 리스크다.
...
| # | 무엇 | 판단 필요 |
|---|---|---|
| 1 | **`LEADGEN_PROVIDER_CONCURRENCY=15`는 실측 전 추정치**다. 20건이 동시에 몰리면 15분 SLA를 못 지킬 수 있다 | 출시 전 부하 시험 |
