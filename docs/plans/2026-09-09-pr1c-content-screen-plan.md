...
**설계 판단(명시):** §4.3은 반려를 없애지만 CLAUDE.md STEP 7과 §2 "공개 후 확인"의 부정 결과를 위해 **표본 글의 확인 흐름 안에서만** "문제 발견 · 비공개 후 재생성"(`POST .../reject`)을 남긴다. 다른 어떤 글에도 반려 버튼은 없다.
...
- [ ] **Step 1: 단위 테스트** — `content_row_state`:
  - PUBLISHED & visible → `("public", None, None)`; PUBLISHED & withheld → `("withheld", "대표 이미지 재인증 대기", None)` (사유 = `visibility.blocker_labels` join)
  - DRAFT with title/body & `compliance.publishable` & `scheduled_date >= today` → `("scheduled", None, None)`
  - DRAFT without title (야간 생성 전) → `("generating", "발행 전날 23:00 자동 생성", None)`; READY 동일
  ...
  - 생성 실패 인시던트가 있으면 `("blocked", "<incident next_action>", {"kind":"incident","href": ...})`; 발행 차단 run(FAILED, `REGENERATE_CONTENT*`)이 있으면 link `{"kind":"run","href":"/operations?queue=incidents&run=<id>"}` — 실제 href는 운영센터 상세 경로(`operations_center_serializers.detail_path` 규칙)를 재사용.
- [ ] **Step 2: 실패 확인.**
- [ ] **Step 3: 구현** — `content_row_state.py`: 위 표를 그대로 코드로; `RowState` frozen dataclass; `ROW_STATE_LABELS = {"public": "공개 중", "withheld": "공개 보류", "scheduled": "예정", "generating": "초안 생성 중", "blocked": "차단", "closed": "종료"}`. `content.py`: 목록/상세 라우트가 월 항목 id로 (a) `Incident.source_id IN ids AND state IN (OPEN, RETRYING)` (기존 `operations_center_today_queries.py:322-333` 술어 재사용), (b) `OperationRun` FAILED with `request_payload.source_id IN ids` and `operation_type IN (REGENERATE_CONTENT, REGENERATE_CONTENT_IMAGE, RECERTIFY_PUBLISHED_IMAGE)` — 각 1쿼리 배치; `_serialize_item(..., blocked_link=...)`에 넘겨 `row_state={"kind","label","reason","link"}` 직렬화. `ContentItemResponse.row_state: dict` 필수. 단건 응답 경로(PATCH/review/reject/get)는 `[item.id]`로 같은 헬퍼.
