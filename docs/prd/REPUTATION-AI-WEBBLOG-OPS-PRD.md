# Re:putation AI 노출 컨설팅·콘텐츠 운영 PRD

작성일: 2026-05-03
제품명: Re:putation
운영사: MotionLabs Inc.
문서 목적: Re:putation의 다음 제품 반복을 "홈페이지 제작"이 아니라 "AI 노출을 위한 컨설팅 기반 콘텐츠 운영"으로 재정의하고, 현재 시스템 위에 필요한 제품/운영/기술 요구사항을 정리한다.

## 1. Executive Summary

Re:putation의 다음 반복은 병원용 홈페이지 제작 상품이 아니다. MotionLabs가 병원별 AI 노출 전략을 세우고, ChatGPT/Gemini 질의에서 병원이 추천·언급될 가능성을 높이기 위해 병원 정보·콘텐츠 허브와 공개 정보 자산을 운영하는 관리형 서비스다.

핵심 고객 약속은 "AI 상위 노출 보장"이 아니라 "환자가 ChatGPT, Gemini 같은 AI에서 병원을 찾을 때, 우리 병원이 추천·노출되도록 돕는 운영 체계"다. 이를 위해 병원 온보딩, source-backed Essence, AI Query Target, SoV 측정, gap diagnosis, query-linked content planning, 콘텐츠 허브 발행, 월간 리포트를 하나의 운영 루프로 묶는다.

이번 PRD의 제품 방향은 기존 콘텐츠 허브를 "홈페이지"로 판매하지 않고, AI가 읽고 인용하기 쉬운 병원 지식 기반 웹블로그로 재정의하는 것이다. Public web surface는 병원 소개 페이지가 아니라 AI 노출 운영의 결과물이자 콘텐츠 허브다.

## 2. Product Positioning

### One-Sentence Definition

Re:putation은 병원이 AI 검색/답변 환경에서 추천 후보로 등장할 수 있도록, 질의 전략 수립부터 근거 기반 콘텐츠 제작, 콘텐츠 허브 발행, SoV 측정, 월간 개선 리포트까지 MotionLabs가 운영하는 컨설팅 기반 콘텐츠 운영 서비스다.

### What It Is / What It Is Not

| 구분 | 정의 |
| --- | --- |
| It is | 병원별 AI 노출 컨설팅, AI 질의 전략, 근거 기반 콘텐츠 철학 수립, 콘텐츠 허브 운영, 구조화 데이터/llms.txt/sitemap 관리, ChatGPT/Gemini SoV 측정, 월간 개선 액션 제안 |
| It is not | 범용 콘텐츠 허브 납품, 병원 랜딩페이지 제작 대행, 단순 블로그 글 대량 생산, SEO 순위 보장 상품, ChatGPT/Gemini 상위 노출 보장 상품, 리뷰 조작/허위 평판 대행 |
...
- "Re:putation은 병원의 AI 노출을 위해 질의 전략, 의료 콘텐츠, 콘텐츠 허브, 구조화 데이터, 월간 리포트를 함께 운영하는 서비스입니다."
...
- 환자 질문과 연결된 콘텐츠 허브 콘텐츠를 매월 기획·생성·검수·발행한다.
...
- Re:putation은 컨설팅 기반 콘텐츠 허브 운영대행이다. 홈페이지 제작 납품이 아니다.
...
- Public web surface는 예쁜 홈페이지가 아니라 AI가 참고할 수 있는 병원 정보·콘텐츠 허브다.
- SoV 실패, 미언급, 경쟁 병원 언급, 출처 링크, 플랫폼 차이는 분리 기록한다.
- 의료광고 리스크, 근거 자료 누락, 최신 승인 Content Philosophy 불일치가 있는 콘텐츠는 자동 발행하지 않는다. 개별 원고는 기계 안전검사 통과 시 예약일에 공개하고, 운영자는 Slack 알림 후 후행 확인한다.
...
- "콘텐츠 허브"는 고객-facing으로 "AI 노출 콘텐츠 허브" 또는 "AI가 참고할 병원 정보·콘텐츠 허브"로 재명명한다.
- Public main page는 콘텐츠 허브 첫 화면이 아니라 병원 엔티티 + 콘텐츠 허브의 entry page로 취급한다.
...
5. Action planning: gap별로 이번 달 실행 액션을 만든다. 예: 콘텐츠 허브 콘텐츠, profile 보강, schema 보강, llms.txt 보강, source refresh, 재측정.
6. Webblog content production: Query Target과 approved Essence를 기반으로 콘텐츠 brief를 만들고 Claude 생성 초안을 만든다.
7. Auto publish/post-review: 시스템이 의료광고, source grounding, 최신 승인 Essence 일치를 재검사한 뒤 예약일 08:00에 자동 발행한다. 운영자는 Slack의 공개/Admin 링크로 후행 확인하고, 문제 발견 시 즉시 수정하거나 비공개 후 재생성한다.
...
- Query Target별로 아래 gap type을 자동 진단한다.
  ...
  - `NO_PUBLIC_CONTENT`: 해당 질의에 대응하는 콘텐츠 허브 콘텐츠 없음
...
- Public site IA를 "홈페이지"에서 "AI 노출 콘텐츠 허브"로 전환한다.
- 기본 IA:
...
- 병원별 `llms.txt`는 병원 facts, 주요 진료항목, 콘텐츠 허브 콘텐츠 URL, query cluster URL, 최신 업데이트 일자를 포함한다.
...
- 리포트는 "월간 SoV 리포트"가 아니라 "AI 노출 콘텐츠 운영 리포트"로 재설계한다.
- 필수 섹션:
  ...
  - Compliance note: 자동 의료광고/근거/Essence 검사 결과, 자동 발행 시각, Slack 알림 시각, 후행 확인자·시각
...
- Dashboard 상단 문구는 "AI 노출 콘텐츠 운영 현황"으로 둔다. "홈페이지 빌드" 중심 표현을 줄인다.
...
- 고객에게는 "콘텐츠 허브 운영"으로 설명하고, "새 홈페이지를 만들어 드립니다"를 주된 가치로 말하지 않는다.

## 14. Report Requirements

리포트 명칭:

- V0: "AI 노출 콘텐츠 운영 진단 리포트"
- 월간: "AI 노출 콘텐츠 운영 리포트"
...
- 원장 상담에서 이해되는 핵심 메시지 비율: "AI 노출 컨설팅·콘텐츠 운영"으로 인식되는지
...
- 고객-facing/관리자-facing 문구를 "AI 노출 컨설팅·콘텐츠 운영"으로 정리.
...
- 문구/네비게이션/리포트 명칭에서 콘텐츠 허브 납품 인상을 제거한다.
- 고객-facing 기본 문장을 "AI 노출 컨설팅·콘텐츠 운영"으로 통일한다.
...
- V0/monthly report를 AI 노출 콘텐츠 운영 리포트로 재설계한다.
...
- Admin, report, customer-facing wording에서 Re:putation이 "AI 노출 컨설팅·콘텐츠 운영"으로 설명된다.
- "콘텐츠 허브 납품" 또는 "AI 노출 보장"으로 오해될 핵심 문구가 제거되거나 금지 문구로 분류된다.
