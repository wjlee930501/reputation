...
test('verify 409 with a prerequisite list is read as missing launch steps', () => {
  const error = new ApiError('운영 시작 전 완료해야 할 단계가 있습니다.', 409, {
    ...
    missing: ['병원 기본 정보 완료', '초기 진단 보고서 생성'],
  })

  const info = readDomainError(error, '도메인 검증에 실패했습니다.')

  assert.equal(info.kind, 'prerequisite')
  assert.deepEqual(info.missingSteps, ['병원 기본 정보 완료', '초기 진단 보고서 생성'])
})

test('extractMissingSteps reads known list keys and object entries with labels', () => {
  assert.deepEqual(extractMissingSteps({ prerequisites: ['콘텐츠 발행 일정 설정'] }), ['콘텐츠 발행 일정 설정'])
  assert.deepEqual(
    extractMissingSteps({ missing_steps: [{ label: '병원 기본 정보 완료' }, { message: '초기 진단 보고서 생성' }] }),
    ['병원 기본 정보 완료', '초기 진단 보고서 생성'],
  )
...
test('parseStepsFromMessage splits a colon-suffixed Korean step list', () => {
  assert.deepEqual(
    parseStepsFromMessage('공개 주소는 확인됐지만 운영 시작 전 단계가 남아 있습니다: 초기 진단 보고서, 콘텐츠 발행 일정'),
    ['초기 진단 보고서', '콘텐츠 발행 일정'],
  )
