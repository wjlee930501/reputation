...
export function ScheduleSection({
  ...
  useEffect(() => {
    ...
    fetchAPI<ScheduleInfo>(`/admin/hospitals/${hospitalId}/schedule`)
      .then((schedule) => {
        if (cancelled || !schedule) return
        setExisting(schedule)
        // 이미 설정된 병원은 월 표부터 본다 — 처음 한 번만 접는다(저장 결과는 계속 보인다).
        setOpen(false)
  ...
  useEffect(() => {
    ...
    fetchAPI<ScheduleReadiness>(`/admin/hospitals/${hospitalId}/readiness`)
      .then((value) => {
      ...
      if (e instanceof ApiError && e.detail && typeof e.detail === 'object') {
        ...
        if (detail.code === 'PLAN_MISMATCH') {
          setError(
            ...
            + ` (계약 ${ADMIN_COPY.plan}: ${detail.contracted_plan ?? '확인 필요'})`,
          )
          return
        }
      }
      setError(safeOperatorError('onboarding', '발행 요일과 시작일을 확인한 뒤 ‘저장’을 다시 누르세요.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    ...
    >
      <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-semibold text-slate-800 [&::-webkit-details-marker]:hidden">
        <span>
          {ADMIN_COPY.schedule} · {existingLoading ? '확인 중' : existing ? '설정됨' : '설정 필요'}
        </span>
        <span aria-hidden className="text-xs text-slate-400">{open ? '접기' : '펼치기'}</span>
      </summary>
      ...
      <div className="border-t border-slate-100 p-4 sm:p-5">
          ...
          <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
            ...
            <dl className="mt-2 space-y-1 text-sm text-blue-900">
              <div className="flex justify-between gap-2">
                <dt className="text-blue-700">{ADMIN_COPY.monthlyArticles}</dt>
        ...
        {result ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* 요금제 — 계약 기록이 정한 값을 읽기 전용으로 보여준다 (H-14) */}
            <div>
              <p className="mb-2 block text-sm font-medium text-slate-700">{ADMIN_COPY.plan}</p>
            ...
            {/* 발행 요일 */}
            <div>
              <span className="mb-2 block text-sm font-medium text-slate-700">발행 요일</span>
            ...
            {/* 시작일 */}
            <div>
              <label htmlFor="schedule-active-from" className="mb-2 block text-sm font-medium text-slate-700">시작일</label>
          ...
          >
            ...
            <dl className="mt-4 divide-y divide-slate-100 rounded-xl border border-slate-200 px-4 text-sm">
              <div className="grid grid-cols-[100px_1fr] gap-3 py-3">
                <dt className="text-slate-500">{ADMIN_COPY.monthlyArticles}</dt>
