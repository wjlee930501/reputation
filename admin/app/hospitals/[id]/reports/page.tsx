...
const LOAD_ERROR: DeliveryIssue = {
  title: '최신 보고서 상태를 불러오지 못했습니다',
  problem: '서버에서 이 보고서의 최신 검수 근거를 확인하지 못했습니다.',
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (active) setPageError('보고서 목록을 불러오지 못했습니다. 고객 영향: 검수와 전달 기록을 확인할 수 없습니다. 지금 할 일: 새로고침 후 계속 실패하면 개발팀에 병원명을 알려 주세요.')
  ...
  async function copyDeveloperInfo() {
    ...
    } catch { setStatusMessage('복사하지 못했습니다. 개발팀에 병원명과 보고서 기간을 알려 주세요.') }
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {selected && <ReportReviewDialog report={selected} issue={issue} isOwner={isOwner} busy={busy} downloadedSha256={downloadedSha256} onDownloaded={setDownloadedSha256} onClose={closeDialog} onRefresh={() => void refreshSelected()} onAction={(action) => void handleAction(action)} onCopyIssue={() => void copyDeveloperInfo()} onCopyNotification={() => void copyDeveloperInfo()} />}
    </div>
  )
}
