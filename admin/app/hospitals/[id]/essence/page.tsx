...
export default function EssencePage() {
  ...
  async function reincludeSource(sourceId: string) {
    setActionLoading(`reinclude-${sourceId}`)
    setError(null)
    setNotice(null)
    try {
      await fetchAPI(`/admin/hospitals/${id}/essence/sources/${sourceId}/reinclude`, { method: 'POST' })
      setSelectedSource(null)
      setNotice('제외를 해제했습니다. 사진 공개 여부는 다시 확인해 주세요.')
      await load()
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : '자료 제외 해제에 실패했습니다.')
    } finally {
      setActionLoading(null)
    }
  }
        ...
        <form onSubmit={createSource} className="bg-white rounded-xl border border-slate-200 p-6 space-y-4 self-start">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            {sourceUrlNotice && <p className="mt-1.5 text-xs text-amber-700">{sourceUrlNotice}</p>}
          </div>
          <div>
                    ...
                    <td className="px-4 py-4 align-top" data-label="액션">
                      <div className="flex flex-wrap items-center justify-end gap-1.5">
                        ...
                        {source.status === 'EXCLUDED' ? (
                          <button
                            onClick={() => reincludeSource(source.id)}
                            disabled={actionLoading === `reinclude-${source.id}`}
                            className="px-2.5 py-1 bg-slate-50 text-slate-700 text-xs rounded hover:bg-slate-100 border border-slate-200 disabled:opacity-50"
                            title="제외를 되돌립니다. 이미 추출한 근거 노트는 재처리 없이 돌아옵니다."
                          >
                            {actionLoading === `reinclude-${source.id}` ? '해제중...' : '제외 해제'}
                          </button>
                        ) : (
                          <button
