...
function deadlineText(item: OperationsQueueRow): string {
  if (item.sla_state === 'NONE' || !item.sla_due_at) return '처리 기한 없음'
  const label = item.sla_state === 'OVERDUE' ? '처리 기한 지남' : '처리 기한 임박'
  return `${label} · ${formatDate(item.sla_due_at)}`
}
...
function ProblemBlock({ item }: { readonly item: OperationsQueueRow }) {
  ...
  return (
    <div className="min-w-0">
      <p className="ops-readable font-semibold text-slate-900">{item.customer.name}</p>
...
function ActionBlock({ item }: { readonly item: OperationsQueueRow }) {
  return (
    <div className="min-w-0 text-sm">
      ...
      <p className={item.sla_state === 'OVERDUE' ? 'mt-1 font-semibold text-red-700' : 'mt-1 text-slate-500'}>
        {deadlineText(item)}
      </p>
    </div>
  )
...
export function OperationsQueueList(props: QueueProps) {
  const { data, visibleItems, loading, error, selectedId, onSelect, onRetryLoad, onPage } = props
  ...
  if (visibleItems.length === 0) {
    return (
      <div className="ops-queue-state">
        ...
        <p className="mt-1 text-sm text-slate-500">필터를 바꾸거나 잠시 후 새로고침해 주세요.</p>
      </div>
    )
  }
      ...
      <div className="ops-cards" aria-label="운영 작업 카드 목록">
        {visibleItems.map((item) => (
          <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 border-t border-slate-100 pt-3"><p className="text-xs font-bold text-slate-500">지금 할 일</p><ActionBlock item={item} /></div>
