...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof BodyTooLargeError) {
      return wantsJson
        ? Response.json({ ok: false, error: 'Payload too large' }, { status: 413 })
        : Response.redirect(new URL('/?lead=invalid#contact', request.url), 303)
    }
    return wantsJson
      ? Response.json({ ok: false, error: 'Invalid form payload' }, { status: 400 })
      : Response.redirect(new URL('/?lead=invalid#contact', request.url), 303)
  }
  ...
  if (
    [lead.question, lead.specialty, lead.region_keyword, ...lead.core_keywords].some(containsPatientSensitiveLeadText)
  ) {
