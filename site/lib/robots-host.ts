 ...
 *
 * 호스트 판정은 middleware·sitemap과 같은 resolveRequestHost를 쓴다 — 세 곳이 서로 다르면
 * 커스텀 도메인으로 서빙된 페이지가 플랫폼 sitemap을 가리키는 어긋난 신호가 나간다.
 */
export function resolveSitemapUrl(
  ...
  if (isPrimaryHost(requestHost, primaryHostnames)) {
  ...
  return `${scheme}://${hostname}${extractPort(requestHost)}/sitemap.xml`
}
