...
const CSS = readFileSync(join(HERE, '..', 'app', '[slug]', '_styles', 'hero.css'), 'utf8')
...
test('the hero renders the separator and lets the browser choose the line breaks', () => {
  ...
  // 조각을 블록으로 쌓으면 조각 수가 그대로 줄 수가 된다 — 기본 규칙에는 블록이 없다.
  assert.doesNotMatch(body, /display:\s*block/)
  assert.doesNotMatch(CSS, /\.hub-hero-title span\b/)
})
...
test('an operator-authored headline gets the block-line modifier, and only it', () => {
  ...
  assert.match(HERO, /hub-hero-title--lines/)

  // 그 모디파이어에만 블록이 걸려 있는가 — 기본 규칙은 위 테스트가 지킨다.
  const modifier = CSS.slice(CSS.indexOf('.hub-hero-title--lines > span {'))
